...
CROPS_DIR = MODELS_DIR / f"{person_name}_crops"
...
X_me = embs[mask_me]
kept, dropped = int(mask_me.sum()), int((~mask_me).sum())
...
np.save(MODELS_DIR / f"{person_name}_centroid.npy", me_centroid)
np.savez(MODELS_DIR / f"{person_name}_gallery.npz",
...
summary = {
    ...
    "centroid_path": str((MODELS_DIR / f"{person_name}_centroid.npy").resolve()),
    "gallery_path": str((MODELS_DIR / f"{person_name}_gallery.npz").resolve()),
    "crops_dir": str(CROPS_DIR.resolve())
}
with open(MODELS_DIR / f"{person_name}_prepare_summary.json", "w") as f:
